...
{
	public Text textContent;    // The display text for the content of the list box
	public Image imageContent;

    public GameObject ingrImage;
	...
	public void ShowBoxInfo()
	{
		Debug.Log("Box ID: " + listBoxID.ToString() +
			...
			", Text: " + _listBank.GetContentText(_contentID));
	}
	...
	void InitialContent()
	{
		...
		_contentID += listBoxID - _positionCtrl.listBoxes.Count / 2;
	...
	void UpdateDisplayContent()
	{
		...
		textContent.text = _listBank.GetContentText(_contentID);
		imageContent.sprite = _listBank.GetContentImage(_contentID);
	...
	void InitialPosition()
	{
		...
		if ((_positionCtrl.listBoxes.Count & 0x1) == 0) {
			switch (_positionCtrl.direction) {
				case ListPositionCtrl.Direction.Vertical:
					transform.localPosition = new Vector3(0.0f,
						_unitPos.y * (listBoxID * -1 + _positionCtrl.listBoxes.Count / 2) - _unitPos.y / 2,
						0.0f);
					UpdateXPosition();
					break;
				case ListPositionCtrl.Direction.Horizontal:
					transform.localPosition = new Vector3(
						_unitPos.x * (listBoxID - _positionCtrl.listBoxes.Count / 2) - _unitPos.x / 2,
						0.0f, 0.0f);
					UpdateYPosition();
					break;
			}
			...
			switch (_positionCtrl.direction) {
				case ListPositionCtrl.Direction.Vertical:
					transform.localPosition = new Vector3(0.0f,
						_unitPos.y * (listBoxID * -1 + _positionCtrl.listBoxes.Count / 2),
						0.0f);
					UpdateXPosition();
					break;
				case ListPositionCtrl.Direction.Horizontal:
					transform.localPosition = new Vector3(
						_unitPos.x * (listBoxID - _positionCtrl.listBoxes.Count / 2),
						0.0f, 0.0f);
					UpdateYPosition();
					break;
			}
